...
<img width="1221" alt="image" src="https://user-images.githubusercontent.com/107796290/205516413-cd5550f0-336b-4070-af87-22c9af6666b4.png">
...
The next process is to compile, train and evaluate the model the dataset. Basic neutral networking with several neurons to create a deeper understanding with hidden layers to deep operational changes to neural networking. This was done within three attempts to compare the accuracy and loss of potential credit risk and investment.
 
 As we review the deep learning in the model performance metric, the model identify who's application is at risk approximately accuracy in the second TensorFlow, optimize the model in order to target the optimal potential outcome. The accuracy was 0.72991251 in the first model.  As you can see the performance of the second and third attempt results are in the summary results. 

Summary results:

In conclusion, the prediction of application type expressed in the table below indicating the type of appplication. In the third deliverable the model is to optimize and prediction of loan accuracy which was targeted hiher than 75%.
 ...
 <img width="1277" alt="image" src="https://user-images.githubusercontent.com/107796290/205516496-4109dedc-e972-428c-965a-03eeda019064.png">
  ...
  The results for the network can evaluate high order nonlinear reoccurrences of the data value weight or density subsequently conducting a potential superior outcome. In the first attempt, the hidden nodes layers are indicated below for two node layers an outer layer. As you can see based on the compilation, train and evaluation model based on the results the overarching first optimal two layers loss: 0.606473982334137, accuracy: 0.7285131216049194 which is close to the target predictive accuracy 75% however not optimized achievement. 
The second attempt used three layers: checkpoints/weights.05.hdf5804/804: loss: 0.5353 - accuracy: 0.7418 for the last 5 epoch was close however the overall loss: 0.5975268483161926, accuracy: 0.727580189704895. The second attempt did not meet the 75% optimization and the loss was a bit higher than the first attempt. 
...
<img width="574" alt="image" src="https://user-images.githubusercontent.com/107796290/205516544-f12a4856-db21-425e-831e-7b2d8e236be9.png">
 ...
 As opposed to the next compilation, train and evaluation model using several nodes and accuracy results were processed with 5 node layers. 
  ...
  In comparing both models predictive accuracy and the calcuations are very similar, either model will be able to prediction the risk of the loan ore lower than 75% of the time. The neural network  can thrive in larger data set similarly to the data we have been using a predictive model across neurons due to deep learning evaulation of every interaction. In turn all three trials to compile, train and evaluate the models did not overperform the 75% accuracy, altough they were close to 75% of the three attempts. 
